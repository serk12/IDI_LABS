...
{
  glm::mat4 TG(1.f);  
  TG = glm::rotate(TG, float(M_PI/6.0f)*rotation, glm::vec3(0,1,0));
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(0.375, -0.4, 0));
  TG = glm::scale(TG, glm::vec3(escalaPat));
  glUniform1i(cowLoc, false);
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::projectTransform ()
{
  glm::mat4 Proj;
  if (perspectiva)
    Proj = glm::perspective(float(M_PI/3.0), ra, 0.1f, 3.0f*radiEsc);
  else
    Proj = glm::ortho(-radiEsc*ra, radiEsc*ra, -radiEsc, radiEsc, 0.1f, 3.0f*radiEsc);
...
{
  glm::mat4 View = glm::lookAt(glm::vec3(-1,1,-1), centrePatr, glm::vec3(0,1,0));
  View = glm::rotate(View   , float(M_PI/6.0f)*rotation, glm::vec3(0,-1,0));
