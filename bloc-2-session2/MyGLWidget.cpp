...
//EX 4 done with homer 
MyGLWidget::MyGLWidget (QWidget* parent) : QOpenGLWidget(parent)
{
  setFocusPolicy(Qt::ClickFocus); 
  scale = 1.0f;
  rotate = 0;
  xClick = yClick = 0;              //EXERCICI 6
  angleY = angleZ = angleX = 0.0;   //EXERCICI 5
...
void MyGLWidget::resizeGL (int w, int h) //EXERCICI 1
{
  ra = float (w / float (h));
  projecTransform();
  glViewport(0, 0, w, h);
...
{
  resizeGL(width(), height()); //EXERCICI 1
...
void MyGLWidget::carregaShaders()
{
  ...
  calcEsfera(glm::vec3 (0,0,0), glm::vec3(0,0,0)); //EXERCICI 2 
  up =  glm::vec3(0,1,0); //ex 3
  projecTransform();                                              
  viewTransform();                                                
}

void MyGLWidget::calcEsfera(glm::vec3 mins, glm::vec3 maxs) { //EXERCICI 2
    if (mins.x == maxs.x and mins.y == maxs.y and mins.z == maxs.z) { //ex 4
        float minx, miny, minz, maxx, maxy, maxz;
        minx = maxx = m.vertices()[0];
        miny = maxy = m.vertices()[1];
        minz = maxz = m.vertices()[2];
        for (unsigned int i = 3; i < m.vertices().size(); i+=3) { //get mins from object
...
void MyGLWidget::viewTransform() {
  glm::mat4 View = glm::lookAt(centerEsfer + (radi*glm::vec3(0,0,1)), centerEsfer,up);//EXERCICI 2
  View = glm::rotate(View, -angleY, glm::vec3(0, 1, 0)); //EXERCICI 5
  View = glm::rotate(View, -angleZ, glm::vec3(0, 0, 1)); //EXERCICI 5
  View = glm::rotate(View, -angleX, glm::vec3(1, 0, 0)); //EXERCICI 5
  glUniformMatrix4fv(viewLoc,1,GL_FALSE, &View[0][0]);
}

void MyGLWidget::projecTransform() {
  glm::mat4 Proj = glm::perspective ((float)3.141516f/2.0f,ra,0.4f,3.0f); //EXERCICI 3 (no modification done)
